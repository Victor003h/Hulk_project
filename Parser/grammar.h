...
class Grammar
{
    public:
        ...
        std::set<Symbol> m_non_terminals;
        ...
        std::map<Symbol, std::set<Symbol>> calculateFirst() const 
        {
            ...
            for (const auto &non_terminal : m_non_terminals) {
            ...
            while (changed) {
                ...
                {
                    const Symbol &A = prod.left; // No terminal de la producción
                    
                    // Caso especial: producción vacía, se interpreta como derivación de ε.
                    // if (prod.right.empty()) {
        ...
        std::map<Symbol, std::set<Symbol>> calculateFollow() const
        {
            std::map<Symbol, std::set<Symbol>> follow;
            for (const auto &non_terminal : m_non_terminals) {
            ...
            follow[m_startSymbol].insert(Symbol("EOFs",SymbolType::Terminal));
        ...
        for (const auto& row : m_parsing_table) {
        ...
        ParseTree parse(std::vector<Token> inpTokens)
        {
            ...
            while(!stack.empty())
            {
                ...
                if(X->m_value->type==SymbolType::NonTerminal)
                {
                    ...
                    if(m_parsing_table[*(X->m_value)].find(lookahead)==m_parsing_table[*(X->m_value)].end())
                    {
                    ...
                    Production p=m_productions[in];
                    
                    for(int i=p.right.size()-1; i>=0; i--)
                    {
                        auto childNode = new ParseNode (&(m_productions[in].right[i]));
